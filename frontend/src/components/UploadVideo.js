...
const UploadVideo = ({ onSubmit }) => {
    ...
    const getVideo = async () => {
        const result = await axios.get("http://localhost:5001/api/getVideos");
    ...
    const submitImage = async (e) => {
        ...
        try {
            const result = await axios.post("http://localhost:5001/api/uploadvideos", formdata, {
    ...
    const showVideo = (video) => {
        window.open(`http://localhost:5001/api/videos/${video}`, "_blank", "noreferrer");
    };
