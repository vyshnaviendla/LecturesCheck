...
const UploadPDF = () => {
    ...
    const getPdf = async () => {
        try {
            const result = await axios.get("http://localhost:5001/api/getfiles");
    ...
    const submitImage = async (e) => {
        ...
        try {
            const result = await axios.post("http://localhost:5001/api/uploadfiles", formData, {
    ...
    const showPdf = (pdf) => {
        window.open(`http://localhost:5001/api/files/${pdf}`, "_blank", "noreferrer");
    };
